...
router.patch("/:id/decrease-stock", (req, res) => {
  const { quantity } = req.body;
  const product = data.products.find((p) => p.id === parseInt(req.params.id));
  if (!product) return res.status(404).json({ error: "Toodet ei leitud" });
  if (product.stock < quantity) return res.status(409).json({ error: "Pole piisavalt laos" });
  
  product.stock -= quantity;
  res.json({ message: "Laoseis uuendatud", product });
});
